...
export const Output = ({ setDisplay, formSummary, cols }: Props) => {
  ...
  const data: SampleStatisticsTableRow[] = (columns as GridColumnName[]).map(
    (colName) => {
      const arrOfNums = cols[colName].filter(Number).map(Number);
      const n = arrOfNums.length;
      const row: SampleStatisticsTableRow = { "": colName };
  ...
  return (
    <>
      ...
      <SampleStatisticsTable data={data} stats={options} />
    </>
  );
};
